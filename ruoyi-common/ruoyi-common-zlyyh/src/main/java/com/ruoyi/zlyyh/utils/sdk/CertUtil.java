...
import static com.ruoyi.zlyyh.utils.sdk.SDKConstants.UNIONPAY_CNNAME;
import static com.ruoyi.zlyyh.utils.sdk.SDKUtil.isEmpty;
...
public class CertUtil {
    ...
    private static void initSignCert() {
        if (!"01".equals(SDKConfig.getSignMethod())) {
            LogUtil.writeLog("非rsa签名方式，不加载签名证书。");
            return;
        }
        if (SDKConfig.getSignCertPath() == null
            || SDKConfig.getSignCertPwd() == null
            || SDKConfig.getSignCertType() == null) {
    ...
    private static void initMiddleCert() {
        LogUtil.writeLog("加载中级证书==>" + SDKConfig.getMiddleCertPath());
        if (!isEmpty(SDKConfig.getMiddleCertPath())) {
            middleCert = initCert(SDKConfig.getMiddleCertPath());
    ...
    private static void initRootCert() {
        LogUtil.writeLog("加载根证书==>" + SDKConfig.getRootCertPath());
        if (!isEmpty(SDKConfig.getRootCertPath())) {
            rootCert = initCert(SDKConfig.getRootCertPath());
    ...
    private static void initEncryptCert() {
        LogUtil.writeLog("加载敏感信息加密证书==>" + SDKConfig.getEncryptCertPath());
        if (!isEmpty(SDKConfig.getEncryptCertPath())) {
            encryptCert = initCert(SDKConfig.getEncryptCertPath());
    ...
    public static PrivateKey getSignCertPrivateKey() {
        try {
            ...
            PrivateKey privateKey = (PrivateKey) keyStore.getKey(keyAlias,
                SDKConfig.getSignCertPwd().toCharArray());
            return privateKey;
    ...
    public static KeyStore getKeyInfo(String pfxkeyfile, String keypwd,
        ...
        try {
            ...
            Resource resource;
            if (pfxkeyfile.indexOf("cert") == 0) {
                resource = new ClassPathResource(pfxkeyfile);
            } else if(pfxkeyfile.indexOf("http") == 0){
                resource = new UrlResource(pfxkeyfile);
            }else {
                resource = new FileUrlResource(pfxkeyfile);
            }
