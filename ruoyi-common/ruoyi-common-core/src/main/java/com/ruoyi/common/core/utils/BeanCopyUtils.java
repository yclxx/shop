...
public class BeanCopyUtils {
    ...
    /**
     * 列表对象基于class创建拷贝 转换为下拉列表
     *
     * @param sourceList 数据来源实体列表
     * @param desc       描述对象 转换后的对象
     * @return desc
     */
    public static <T> List<SelectListEntity> copyListToSelectListVo(List<T> sourceList, String idName, String labelName, String rightLabelName) {
        if (ObjectUtil.isNull(sourceList)) {
            return null;
        }
        if (CollUtil.isEmpty(sourceList)) {
        ...
        if(StringUtils.isNotBlank(rightLabelName)){
            map.put(rightLabelName, "rightLabel");
        }
        CopyOptions copyOptions = CopyOptions.create().setFieldMapping(map);
        return BeanUtil.copyToList(sourceList, SelectListEntity.class, copyOptions);
    }
