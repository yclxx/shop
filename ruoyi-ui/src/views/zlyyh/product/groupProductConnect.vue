<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button v-show="isProductGroup === true" plain size="mini" @click="handleProductGroup(1)">选择商品
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-show="isProductGroup === true" :loading="buttonLoading" type="danger" plain
                   icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleProductProductGroup(0)">解除关联
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-show="isProductGroup === false" plain size="mini" @click="handleProductGroup(0)">查看商品
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-show="isProductGroup === false" :loading="buttonLoading" type="primary" plain
                   icon="el-icon-check" size="mini" :disabled="multiple" @click="handleProductProductGroup(1)">绑定关联
        </el-button>
      </el-col>
    </el-row>
...
<script>
import {groupProductList} from "@/api/zlyyh/product";
import {selectListPlatform} from "@/api/zlyyh/platform"
import {updateGroupProduct} from "@/api/zlyyh/productGroup"
import item from "@/layout/components/Sidebar/Item.vue";

export default {
  name: "groupProductConnect",
  components: {},
  computed: {
    item() {
      return item
  ...
  methods: {
    ...
    handleProductGroup(index) {
      this.isProductGroup = (index === 0);
      this.handleQuery();
    },
    /** 查询商品列表 */
    getList() {
      ...
      if (this.productGroupId && this.productGroupId !== '') {
        this.queryParams.productGroupId = this.productGroupId;
      ...
      groupProductList(this.queryParams).then(response => {
    ...
    handleProductProductGroup(type) {
      const param = {
        'productIds': this.ids,
        'productGroupId': this.productGroupId,
        'type': type
      }
      updateGroupProduct(param).then(response => {
        if (type === 1) {
