      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['zlyyh:order:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-edit-outline" @click="handlePushLog(scope.row)"
                     v-hasPermi="['zlyyh:order:pushLog']">取码记录</el-button>
...
<!--                     v-hasPermi="['zlyyh:order:reissue']">补发</el-button>-->
          <el-button
...
<script>
import { listHistoryOrder, getHistoryOrder, delHistoryOrder, addHistoryOrder, updateHistoryOrder } from "@/api/zlyyh/historyOrder";
import {
  selectListPlatform
} from "@/api/zlyyh/platform";
import {
  listHistoryOrderPushInfo,

} from "@/api/zlyyh/historyOrderPushInfo";
import {
  Row
} from "element-ui";
import {
  addOrderBackTransHistory,
  addOrderBackTransDirectHistory
} from "@/api/zlyyh/orderBackTrans";
export default {
  name: "HistoryOrder",
  dicts: ['t_order_status', 't_order_pickup_method', 't_order_send_status', 't_order_push_status', 't_product_type','t_cus_refund'],
  data() {
    return {
      refundWay: '0',
      pickupMethod: '',
  ...
  methods: {
    ...
    getList() {
      ...
      listHistoryOrder(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getHistoryOrder(number).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.number != null) {
            updateHistoryOrder(this.form).then(response => {
            ...
            addHistoryOrder(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除历史订单编号为"' + numbers + '"的数据项？').then(() => {
        this.loading = true;
        return delHistoryOrder(numbers);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('zlyyh/historyOrder/export', {
        ...this.queryParams
    ...
    getOrderPushInfoList() {
      ...
      listHistoryOrderPushInfo(this.orderPushInfo.queryParams).then(response => {
    ...
    submitRefundForm() {
      this.$refs["refundForm"].validate(valid => {
        if (valid) {
          this.buttonLoading = true;
          if (this.refundWay == '0') {
            addOrderBackTransHistory(this.refundForm).then(response => {
            ...
            addOrderBackTransDirectHistory(this.refundForm).then(response => {
              this.$modal.msgSuccess("操作成功");
              this.refundOpen = false;
              this.getList();
            }).finally(() => {
