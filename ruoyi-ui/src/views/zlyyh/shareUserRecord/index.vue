  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="被分销用户ID" prop="inviteeUserId">
        <el-input
          v-model="queryParams.inviteeUserId"
          placeholder="请输入被分销用户ID"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="订单核销时间" prop="orderUsedTime">
        <el-date-picker clearable
          v-model="queryParams.orderUsedTime"
          type="date"
      ...
      <el-form-item label="分销状态" prop="inviteeStatus">
        <el-select v-model="queryParams.inviteeStatus" placeholder="请选择分销状态" clearable>
          <el-option
            v-for="dict in dict.type.invitee_status"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="奖励状态" prop="awardStatus">
        <el-select v-model="queryParams.awardStatus" placeholder="请选择奖励状态" clearable>
          <el-option
            v-for="dict in dict.type.award_status"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="奖励发放账号" prop="awardAccount">
        <el-input
          v-model="queryParams.awardAccount"
          placeholder="请输入奖励发放账号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['zlyyh:shareUserRecord:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['zlyyh:shareUserRecord:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['zlyyh:shareUserRecord:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['zlyyh:shareUserRecord:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="shareUserRecordList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="ID" align="center" prop="recordId" v-if="true"/>
      <el-table-column label="分销员用户ID" align="center" prop="userId" />
      <el-table-column label="被分销用户ID" align="center" prop="inviteeUserId" />
      <el-table-column label="订单号" align="center" prop="number" />
      <el-table-column label="订单核销时间" align="center" prop="orderUsedTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.orderUsedTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="奖励发放账号" align="center" prop="awardAccount" />
...
<script>
import { listShareUserRecord, getShareUserRecord, delShareUserRecord, addShareUserRecord, updateShareUserRecord } from "@/api/zlyyh/shareUserRecord";
  ...
  data() {
    return {
      ...
      rules: {
        ...
        userId: [
          { required: true, message: "分销员用户ID不能为空", trigger: "blur" }
        ],
        inviteeUserId: [
          { required: true, message: "被分销用户ID不能为空", trigger: "blur" }
        ],
        number: [
