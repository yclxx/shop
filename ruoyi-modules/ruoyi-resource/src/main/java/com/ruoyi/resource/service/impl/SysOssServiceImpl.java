...
import com.ruoyi.resource.config.FileConfig;
...
//        result.setRecords(filterResult);
    ...
    @Override
    public SysOssVo downOss(Long ossId) {
        SysOssVo sysOss = SpringUtils.getAopProxy(this).getById(ossId);
        if (ObjectUtil.isNull(sysOss)) {
            throw new ServiceException("文件数据不存在!");
        }
    ...
    @Override
    public SysOssVo uploadLocalhost(MultipartFile file) {
        String configKey = RedisUtils.getCacheObject(OssConstant.DEFAULT_CONFIG_KEY);
        String bucketName = FileConfig.getDomain() + FileConfig.getPrefix();
        String fileName = FileConfig.getPath();
        Map<String, Object> map;
        String extension;
        try {
            map = FileUploadUtils.upload(fileName, file);
        ...
        oss.setUrl(bucketName + "/" + fileName);
