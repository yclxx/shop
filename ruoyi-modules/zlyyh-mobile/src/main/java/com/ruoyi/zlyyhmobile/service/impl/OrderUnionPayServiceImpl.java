...
public class OrderUnionPayServiceImpl implements IOrderUnionPayService {
    ...
    private LambdaQueryWrapper<OrderUnionPay> buildQueryWrapper(OrderUnionPayBo bo) {
        ...
        lqw.eq(StringUtils.isNotBlank(bo.getUsrPayAmt()), OrderUnionPay::getUsrPayAmt, bo.getUsrPayAmt());
        lqw.eq(StringUtils.isNotBlank(bo.getPayTn()), OrderUnionPay::getPayTn, bo.getPayTn());
        lqw.eq(StringUtils.isNotBlank(bo.getPayTxnTime()), OrderUnionPay::getPayTxnTime, bo.getPayTxnTime());
        lqw.eq(StringUtils.isNotBlank(bo.getPayUrl()), OrderUnionPay::getPayUrl, bo.getPayUrl());
        lqw.eq(StringUtils.isNotBlank(bo.getSettleDt()), OrderUnionPay::getSettleDt, bo.getSettleDt());
        lqw.eq(StringUtils.isNotBlank(bo.getSettleAmt()), OrderUnionPay::getSettleAmt, bo.getSettleAmt());
        lqw.eq(StringUtils.isNotBlank(bo.getSettleCurrencyCode()), OrderUnionPay::getSettleCurrencyCode, bo.getSettleCurrencyCode());
        lqw.eq(StringUtils.isNotBlank(bo.getSettleKey()), OrderUnionPay::getSettleKey, bo.getSettleKey());
        return lqw;
    }
